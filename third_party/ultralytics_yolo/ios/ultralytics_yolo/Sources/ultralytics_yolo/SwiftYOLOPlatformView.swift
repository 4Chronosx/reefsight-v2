  ...
  @preconcurrency FlutterStreamHandler
{
  ...
  private func setupMethodChannel() {
    ...
    methodChannel.setMethodCallHandler { [weak self] (call, result) in
      ...
      case "startRecording":
        // Sub-plan 3 step 1 ("Recording/inference isolation") -- see
        // VideoCapture.swift's `movieOutput` for the design. result(nil)
        // fires once the record command is accepted, not once the file is
        // finished (that only happens at stopRecording) -- the completion
        // here is native-side error logging only.
        if let args = call.arguments as? [String: Any],
          let path = args["path"] as? String
        {
          self.yoloView?.startRecording(to: path) { error in
            if let error {
              NSLog("YOLO SwiftYOLOPlatformView: recording finished with error: %@", error.localizedDescription)
            }
          }
          result(nil)  // Success
        } else {
          result(
            FlutterError(
              code: "invalid_args", message: "Invalid arguments for startRecording", details: nil))
        }

      case "stopRecording":
        // result(nil) fires only once the file has actually finished
        // writing (see VideoCapture.stopRecording's completion chaining),
        // so a Dart caller awaiting this can safely read the file right
        // after.
        self.yoloView?.stopRecording { error in
          if let error {
            result(
              FlutterError(
                code: "recording_error", message: error.localizedDescription, details: nil))
          } else {
            result(nil)
          }
        }
