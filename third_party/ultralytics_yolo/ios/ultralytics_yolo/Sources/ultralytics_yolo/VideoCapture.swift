...
class VideoCapture: NSObject, @unchecked Sendable {
  ...
  // Sub-plan 3 step 1 ("Recording/inference isolation"): a second output on
  // this SAME captureSession, independent of videoOutput's sample-buffer
  // delegate (and therefore independent of inference/classification running
  // on that delegate's queue). AVCaptureMovieFileOutput writes via its own
  // internal encoder path, so it isn't subject to videoOutput's
  // alwaysDiscardsLateVideoFrames contention -- a slow or throwing
  // inference pass on the Dart side can't stall or lose recorded frames.
  // Residual risk (unchanged from before this existed): a full native
  // process crash can still lose an unfinalized recording -- accepted per
  // ReefSight_Specification.md "Recording & crash resilience", not
  // engineered around further.
  let movieOutput = AVCaptureMovieFileOutput()
  private var recordingCompletion: ((Error?) -> Void)?
  ...
  func setUpCamera(
    ...
    // See the `movieOutput` declaration above for why this is a second,
    // independent output rather than routed through videoOutput's delegate.
    if captureSession.canAddOutput(movieOutput) {
      captureSession.addOutput(movieOutput)
    } else {
      NSLog("YOLO VideoCapture: Cannot add movie file output to capture session")
    }
  ...
  /// Starts writing `movieOutput`'s output to `url`, independent of
  /// inference. `completion` fires once (on the main queue) when the file
  /// finishes writing -- either from `stopRecording()` or a recording
  /// error -- matching `AVCaptureFileOutputRecordingDelegate`'s single
  /// completion callback.
  func startRecording(to url: URL, completion: @escaping (Error?) -> Void) {
    guard captureSession.isRunning else {
      DispatchQueue.main.async { completion(NSError(
        domain: "YOLOVideoCapture", code: 1,
  ...
  /// Stops the current recording, if any. `completion` fires once the file
  /// has actually finished writing (not merely once `stopRecording` was
  /// called) -- chained onto whatever completion `startRecording` was given,
  /// since `AVCaptureFileOutputRecordingDelegate` only fires once per
  /// recording regardless of which caller is waiting on it.
  func stopRecording(completion: ((Error?) -> Void)? = nil) {
    cameraQueue.async { [weak self] in
      ...
      if let completion {
        let previous = self.recordingCompletion
        self.recordingCompletion = { error in
          previous?(error)
          completion(error)
        }
      }
...
extension VideoCapture: AVCaptureFileOutputRecordingDelegate {
  func fileOutput(
    ...
    DispatchQueue.main.async { completion(errorBox.value) }
  }
}
