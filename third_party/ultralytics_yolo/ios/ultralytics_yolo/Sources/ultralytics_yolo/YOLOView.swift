...
public class YOLOView: UIView, VideoCaptureDelegate {
  ...
  // MARK: - Recording (sub-plan 3 step 1, "Recording/inference isolation")
  //
  // Thin passthrough to videoCapture's AVCaptureMovieFileOutput -- see
  // VideoCapture.swift's `movieOutput` for why this is a second, independent
  // output on the same capture session rather than derived from the
  // inference-throttled frame stream.

  /// Starts recording the live camera feed to `path`, independent of
  /// inference. `completion` fires once the file has finished writing (on
  /// `stopRecording()`, or immediately with an error if recording could not
  /// start/finish cleanly).
  public func startRecording(to path: String, completion: @escaping (Error?) -> Void) {
    videoCapture.startRecording(to: URL(fileURLWithPath: path), completion: completion)
  }

  /// Stops the current recording, if any. `completion` fires once the file
  /// has actually finished writing.
  public func stopRecording(completion: ((Error?) -> Void)? = nil) {
